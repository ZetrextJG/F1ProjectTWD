...
res <- rbind(max_air_temp, max_wind_speed, max_rain, low_pressure)
...
graph_data %>% 
  ...
  theme(
    ...
    legend.text = element_text(size = 28, family = "roboto", color = "#dfe6e9"),
    legend.key = element_rect(fill = NA, color = NA),
    legend.background = element_blank()
  ) + 
  labs(title = "Percentage of points", subtitle = "gather by teams in differently conditioned races") + 
  theme(
...
plt$layers[[1]]$aes_params <- c(plt$layers[[1]]$aes_params, colour = "#d2dae2")
plt$layers[[5]]$aes_params <- c(plt$layers[[5]]$aes_params, colour = "#d2dae2")
plt$layers[[6]]$aes_params <- c(plt$layers[[6]]$aes_params, colour = "#d2dae2")
plt

ggsave(
  filename = here::here("graphs", "radar.png"),
  plot = plt,
  width = 6,
  height = 5,
  device = "png"
